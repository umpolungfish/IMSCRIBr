...
# ── SIXTEEN_3 trilattice lane offsets ──────────────────────────────────────────────
Y_I      = 260   # information lane (≤_i) — between T and main
Y_C      = 400   # constructivity lane (≤_c) — between main and F
NODE_R   = 22
...
class DiagramLayout:
    ...
    dynamic_width:  Optional[float] = None       # expanded SVG width when tokens would overlap
    i_owner:     Dict[int, int] = field(default_factory=dict)  # I-branch ownership
    trilattice_data: Optional[dict] = None           # SIXTEEN_3 trace data
...
def compute_layout(graph: WiredGraph) -> DiagramLayout:
    ...
    dynamic_width: Optional[float] = None

    # ── OVERLAP PREVENTION: if natural span is smaller than minimum node spacing,
    #     expand the diagram horizontally so tokens never overlap. ──
    if span < MIN_NODE_SPACING:
        span = MIN_NODE_SPACING
        dynamic_width = 2 * MARGIN_X + n_tokens * span
        dynamic_width = max(dynamic_width, SVG_W)  # never shrink below default
        usable_w = dynamic_width - 2 * MARGIN_X
    ...
    return DiagramLayout(
        ...
        i_owner=i_owner,
        trilattice_data=getattr(graph, 'trilattice_data', None),
    )
...
def render_wiring_svg_v3(graph: WiredGraph, name: str = "", ourobor: str = "",
                          ...
                          pen_mode: bool = False, topology_report: dict = None, trilattice_data: dict = None) -> SVGBuilder:
    """
    ...
    if pen_mode:
        return render_wiring_pen_svg(graph, name, ourobor, description, topology_report, trilattice_data)
    ...
    dw = layout.dynamic_width or SVG_W
    svg = SVGBuilder(w=dw)
    ...
    svg.text(dw/2, 26, title, 13, FG, "middle", True)
    if description:
        svg.text(dw/2, 41, description[:155], 7, "#777", "middle")
    ...
    # ── Footer ──
    if ig_type:
        svg.text(dw/2, SVG_H - 22, ig_type, 7, "#5577aa", "middle", False, "monospace")
    if ourobor:
        svg.text(dw/2, SVG_H - 9, f"Ouroboricity: {ourobor}", 7, "#9988aa", "middle", False, "serif")
    ...
    # ── Ouroboric back-arc (DRAWN LAST — on top of all elements) ──
    if ourobor and ourobor != "O₀" and len(pos) >= 2:
        ...
        if i0 in pos and iN in pos:
            ...
            _bt = NODE_R + PORT_GAP
            _sx = x0 + _bux * _bt; _sy = y0 + _buy * _bt
            _ex = xn - _bux * _bt; _ey = yn - _buy * _bt
            # Arc peak well above all content (title is at y=26, T-lane starts at y=135)
            _arc_peak = 10
            _mid_x = (x0 + xn) / 2
            _path = f"M{_ex:.1f},{_ey:.1f} L{_ex:.1f},{_arc_peak:.1f} L{_sx:.1f},{_arc_peak:.1f} L{_sx:.1f},{_sy:.1f}"
            svg.add("path", {"d": _path, "fill": "none", "stroke": BACK_COLOR,
                             "stroke-width": "1.8", "opacity": "0.75"})
            # Arrowhead pointing into return node
            svg.add("polygon", {
                "points": _arrow_head_points(_sx, _sy, _sx, _arc_peak, 9),
                "fill": BACK_COLOR, "opacity": "0.75"
            })
            svg.text(_mid_x, _arc_peak + 2, ourobor, 8, "#9988cc", "middle", False, "serif")
    svg.close()
    return svg
...
def render_wiring_pen_svg(graph: WiredGraph, name: str = "", ourobor: str = "",
                          description: str = "", topology_report: dict = None, trilattice_data: dict = None) -> SVGBuilder:
    ...
    layout = compute_layout(graph)
    layout.trilattice_data = trilattice_data
    tokens, n, states, pos = layout.tokens, layout.n, layout.states, layout.pos

    pdw = layout.dynamic_width or PEN_W
    svg = SVGBuilder(w=pdw)
    svg.parts[1] = f'<rect width="{pdw}" height="{SVG_H}" fill="#000000"/>'
    ...
    # ── faint grid ──
    for gx in range(0, int(pdw), GRID_SPACING):
        svg.line(gx, 0, gx, SVG_H, stroke=PEN_GRID, width=0.4, opacity=0.3)
    for gy in range(0, SVG_H, GRID_SPACING):
        svg.line(0, gy, pdw, gy, stroke=PEN_GRID, width=0.4, opacity=0.3)

    # ── title ──
    title = name.replace("_", " ") if name else ""
    svg.text(pdw/2, 26, title, 13, PEN_INK, "middle", True)
    if description:
    ...
    # ── footer ──
    if ourobor:
        svg.text(pdw/2, SVG_H - 12, f"Ouroboricity: {ourobor}", 7, PEN_INK, "middle")

    # ── ouroboric closure arc (DRAWN LAST — on top of all elements) ──
    if len(pos) >= 2:
        last = max(pos)
    ...
    # ── SIXTEEN_3 trilattice info panel ──
    _pen_trilattice_panel(svg, layout, pdw)
    svg.close()
    return svg
...
def _pen_trilattice_panel(svg: 'SVGBuilder', layout, w: float):
    """Draw SIXTEEN_3 trilattice info panel at bottom-right of diagram."""
    tri = getattr(layout, 'trilattice_data', None)
    if tri is None:
        return
    # Panel position — bottom right
    bx, by, bw, bh = w - 340, SVG_H - 95, 320, 80
    svg.add("rect", {"x": f"{bx:.1f}", "y": f"{by:.1f}", "width": f"{bw:.1f}", "height": f"{bh:.1f}",
                     "fill": "#000000", "stroke": PEN_INK, "stroke-width": "0.5", "opacity": "0.9"})
    svg.text(bx + 6, by + 10, "SIXTEEN₃ TRILATTICE", 6, PEN_INK, "start", True)
    # 16_3 register sequence
    reg_seq = tri.get("register_seq", [])
    if reg_seq:
        seq_str = " → ".join(reg_seq[:24])
        if len(reg_seq) > 24:
            seq_str += " → ..."
        svg.text(bx + 6, by + 22, f"16₃ reg: {seq_str}", 4.5, PEN_INK, "start")
